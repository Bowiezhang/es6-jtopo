...
!
    function (window) {
        ...
        CanvasRenderingContext2D.prototype.JTopoRoundRect = function (a, b, c, d, e) {
            ...
            MouseCursor: {
                ...
                // open_hand: "url("+'./image/'+"openhand.cur) 8 8, default",
                // closed_hand: "url("+'./image/'+"closedhand.cur) 8 8, default"
            },
            replaceStageWithJson: function (jsonObj, stage, scene, config) {
                var stage = stage;
            ...
            createStageFromJson: function (jsonObj, canvas, config) {
    ...
    function (JTopo) {
        ...
        function Stage(c) {
            ...
            q.forEach(function (a) {
            ...
            }, this.getImageFile = function (a, b) {
                var c = this.eagleEye.getImage(a, b);
                var arr = c.split(','),
                    mime = arr[0].match(/:(.*?);/)[1],
                    bstr = atob(arr[1]),
                ...
                this.childs.forEach(function (a) {
                    0 != a.visible && a.childs.length > 0 && a.centerAndZoom()
                })
    ...
    function (JTopo) {
        function Scene(c) {
            ...
            this.initialize = function () {
                ...
                var c = "translateX,translateY,scaleX,scaleY,backgroundColor".split(",");
    ...
    function (a) {
        ...
        function d() {
            ...
            this.paintCtrl = function (a) {
                ...
                if (null == this.selectedPoint) {
                    var b = this.selectedLocation.x + a.dx,
                        c = this.selectedLocation.y + a.dy;
                    // 自动定位
                    let nodeLocation = this.automaticLayout(b, c, this)
                    if (nodeLocation.x) { b = nodeLocation.x }
                    if (nodeLocation.y) { c = nodeLocation.y }
                    // 应用自动布局
            ...
            }, this.automaticLayout = function(b, c, node) {
                let x, y
                const nodeList = JTopo.stage.childs[0].childs.filter(
                    (item) => item.elementType === 'node',
                )
                for (let index = 0; index < nodeList.length; index++) {
                    if (nodeList[index] === node) {
                        continue
                    }
                    if (nodeList[index].x + 5 > b && nodeList[index].x - 5 < b) {
                        x = nodeList[index].x
                    }
                    if (nodeList[index].y + 5 > c && nodeList[index].y - 5 < c) {
                        y = nodeList[index].y
                    }
                    if (x && y) {
                        return { x, y }
    ...
    function (a) {
        function b(c) {
            this.initialize = function (c) {
                b.prototype.initialize.apply(this, arguments), this.elementType = "node", this.zIndex = a.zIndex_Node, this.text = c, this.font = "12px Consolas", this.fontColor = "255,255,255", this.borderWidth = 0, this.borderColor = "255,255,255", this.borderRadius = null, this.dragable = !0, this.textPosition = "Bottom_Center", this.textOffsetX = 0, this.textOffsetY = 0, this.transformAble = !0, this.inLinks = [], this.outLinks = [], this.extData = null, this.onlyConnect = false;
                var d = "nodeId,nodeType,nodeParams,nodeImage,text,textPosition,layout,fontColor,extData,onlyConnect".split(",");
            ...
            }, CanvasRenderingContext2D.prototype.wrapText = function (textPosition, str, x, y) {
                ...
                if (textPosition === "Top_Left" || textPosition === "Middle_Left") {
                    x = x - maxWidth / 2
                    y = y - (textArray.length - 1) * lineHeight / 2
                } else if (textPosition === "Top_Center" || textPosition === "Top_Right" || textPosition === "Middle_Center" || textPosition === "Middle_Right") {
                    y = y - (textArray.length - 1) * lineHeight / 2
                }
                ...
                if (null != b && "" != b) {
                    ...
                    a.wrapText(this.textPosition, b, e.x, e.y), a.closePath()
                }
        ...
        function f(b, c, g) {
            ...
            this.initialize = function (b, c, d) {
                if (f.prototype.initialize.apply(this, arguments), this.elementType = "link", this.zIndex = a.zIndex_Link, 0 != arguments.length) {
                    ...
                    var e = "nodeSrc,nodeDst,lineType,text,linkParams,direction,strokeColor,PointPathColor,lineWidth".split(",");
                ...
                for (var c = 1; c < b.length; c++) {
                    null == this.dashedPattern ? (
                        (null == this.PointPathColor ? a.lineTo(b[c].x, b[c].y) : a.JtopoDrawPointPath(b[c - 1].x, b[c - 1].y, b[c].x, b[c].y, a.strokeStyle, this.PointPathColor, c - 1))
        ...
        function h(a, b, c) {
            this.initialize = function () {
                ...
                var d = [],
                    ...
                    i = "horizontal" == this.direction ? Math.abs((c.x - b.x) / 2) : Math.abs((c.y - b.y) / 2);
    ...
    function (a) {
        ...
        function adjustPosition(currNode, nodeChilds, event) {
            if (currNode.layout && currNode.layout.on) {
                ...
                //         jtopo_editor.lastMovedX = currNode.selectedLocation.x + event.dx - currNode.x;
                //         jtopo_editor.lastMovedY = currNode.selectedLocation.y + event.dy - currNode.y;
                //     }
                //     // 子节点同步移动
                //     nodeChilds.forEach(function (n) {
                //         // 对于有多个节点指向的情况，为避免重复位移，每次移动幅度按比例缩小
                //         n.setLocation(n.x + jtopo_editor.lastMovedX / (n.inLinks.length > 0 ? n.inLinks.length : 1), n.y + jtopo_editor.lastMovedY / (n.inLinks.length > 0 ? n.inLinks.length : 1));
                //     });
                // }
